...
lesson_type_enum = ENUM("article", "video", "colab", "quiz", name="lesson_type_enum", create_type=True)
...
class LearningPath(Base):
    ...
    # Relationships
    modules = relationship("PathModule", back_populates="path", lazy="selectin", order_by="PathModule.display_order")
...
class PathModule(Base):
    ...
    lessons = relationship("PathLesson", back_populates="module", lazy="selectin", order_by="PathLesson.display_order")
...
class PathLesson(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    module_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("path_modules.id"), nullable=False)
    title: Mapped[str] = mapped_column(String(200), nullable=False)
