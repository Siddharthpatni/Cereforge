...
(async () => {
    ...
    await page.evaluate((data) => {
        localStorage.setItem('auth-storage', JSON.stringify({
            state: { isAuthenticated: true, user: data.user, token: data.access_token },
            version: 0
        }));
    }, authData);
    ...
    await page.goto('http://localhost:5173/tasks/llm-prompt-chain');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000); // let task detail render
    await page.screenshot({ path: path.join(__dirname, '../docs/screenshots/task-detail.png') });
    console.log('Saved task-detail.png');

    console.log('Navigating to leaderboard...');
    await page.goto('http://localhost:5173/leaderboard');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000); // let leaderboard render
    await page.screenshot({ path: path.join(__dirname, '../docs/screenshots/leaderboard.png') });
    ...
    await mobilePage.screenshot({ path: path.join(__dirname, '../docs/screenshots/mobile.png') });
