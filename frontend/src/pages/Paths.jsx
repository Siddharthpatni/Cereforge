...
export function Paths() {
    ...
    if (loading) return <div className="p-12 text-center text-primary animate-pulse">Loading Curriculums...</div>;

    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold text-white tracking-tight">Structured Learning Paths</h1>
                <p className="text-zinc-400 mt-1">Master specific domains of AI through curated sequences of challenges.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {paths.map(path => {
                    const isEnrolled = path.student_progress !== null;
                    const completed = isEnrolled ? path.student_progress.completed_tasks : 0;
                    const total = path.total_tasks;
                    const progressPerc = total > 0 ? (completed / total) * 100 : 0;

                    return (
                        <Card key={path.id} className="flex flex-col h-full glass-panel hover:border-primary/50 transition-colors">
                            <CardHeader>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-primary font-mono mt-1">Difficulty: <span className="capitalize">{path.difficulty}</span></p>
                                    </div>
                                </div>
                            </CardHeader>
                                    ...
                                    {isEnrolled ? (
                                        <div className="w-full">
                                            ...
                                            <div className="w-full bg-zinc-800 rounded-full h-2 overflow-hidden mb-4">
                                                <div className="bg-success h-2 rounded-full transition-all duration-500" style={{ width: `${progressPerc}%` }} />
                                            </div>
