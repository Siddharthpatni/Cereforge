...
export function WeeklyTasks() {
    ...
    const fetchTasks = async () => {
        ...
        try {
            const url =
                selectedTrack === "all" ? "/tasks?is_weekly=true" : `/tasks?is_weekly=true&track=${selectedTrack}`;
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-2">
                        Weekly Challenges <span className="text-xl">⚔️</span>
                    </h1>
                    <p className="text-zinc-400 mt-1">
                        Complete these highly challenging weekly tasks to earn the rare Weekly Warrior badge!
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center p-12 border border-dashed border-zinc-800 rounded-xl bg-zinc-900/20">
                    <p className="text-zinc-400">
                        No weekly tasks found passing the filter. Wait until next week to see new tasks!
                    </p>
                    <Button
                        variant="ghost"
                        className="mt-4"
