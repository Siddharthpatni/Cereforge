...
function CommentNode({ comment, postAuthorId, onVote, onAccept, onReply }) {
    ...
    return (
        <div className={cn("flex flex-col gap-2 mt-4", comment.parent_id ? "pl-6 sm:pl-10 border-l border-border/50" : "")}>
            <div className={cn(
                "flex gap-4 p-4 rounded-xl border transition-colors",
                comment.is_accepted ? "bg-success/5 border-success/30" : "bg-zinc-900/30 border-border/50"
            )}>
                {/* Voting */}
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                            ...
                            {comment.author_id === postAuthorId && <Badge variant="outline" className="text-[9px] py-0 px-1 border-primary/50 text-primary">OP</Badge>}
                            <span className="text-xs text-zinc-500">• {formatDistanceToNow(new Date(comment.created_at))} ago</span>
                        </div>
                        {comment.is_accepted && (
                            <span className="flex items-center gap-1 text-success text-xs font-bold"><CheckCircle className="h-4 w-4" /> Accepted Answer</span>
                        )}
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-3">
                        <button onClick={() => setShowReplyBox(!showReplyBox)} className="text-xs font-medium text-zinc-400 hover:text-white transition-colors">
                            Reply
                        </button>
                    ...
                    {showReplyBox && (
                        <form onSubmit={handleReplySubmit} className="mt-3 relative">
                            ...
                            />
                            <Button type="submit" size="sm" isLoading={submitting} className="absolute right-2 bottom-3">Submit</Button>
                        </form>
                    )}
                </div>
            </div>
...
export function PostDetail() {
    ...
    const fetchPost = async () => {
        try {
            ...
            addToast({ title: 'Error', message: 'Discussion not found', type: 'error' });
            navigate('/community');
        } finally {
            setLoading(false);
        }
    ...
    const handleVote = async (id, value, targetType = 'post') => {
        try {
            ...
            addToast({ title: 'Vote Failed', message: err.response?.data?.detail || 'Try again.', type: 'warning' });
        }
    };

    const handleAcceptAnswer = async (commentId) => {
        try {
            await apiClient.post(`/community/posts/${postId}/accept?comment_id=${commentId}`);
            addToast({ title: 'Answer Accepted', message: 'You have rewarded the author with XP.', type: 'success' });
            fetchPost();
        } catch (err) {
            addToast({ title: 'Error', message: 'Failed to accept answer', type: 'error' });
        }
    };
    ...
    const handlePostComment = async (parentId, content) => {
        try {
            ...
            addToast({ title: 'Posted', message: 'Your comment was posted.', type: 'success' });
            fetchPost();
        } catch (err) {
    ...
    const requestAISummary = async () => {
        setSummarizing(true);
        try {
            const res = await apiClient.post(`/ai/mentor/community-assist?post_id=${postId}`);
            setAiSummary(res.data.summary || res.data); // depending on backend structure
        } catch (err) {
            addToast({ title: 'AI Unavailable', message: 'Failed to generate summary.', type: 'error' });
        } finally {
            setSummarizing(false);
        }
    };

    if (loading) return <div className="p-12 text-center text-primary animate-pulse">Loading Discussion...</div>;
    if (!post) return null;
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto pb-12">
            {/* Header */}
            <div>
                <Button variant="ghost" size="sm" onClick={() => navigate('/community')} className="text-zinc-400 hover:text-white px-0 hover:bg-transparent -ml-2 mb-4">
                    <ArrowLeft className="h-4 w-4 mr-1" /> Back to Community
                </Button>
                <h1 className="text-3xl font-bold text-white mb-3 tracking-tight">
                    {post.is_resolved && <span className="text-success mr-2">[Resolved]</span>}
                    {post.title}
                </h1>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-zinc-500 border-b border-border/50 pb-6">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-zinc-300 font-medium">u/{post.author?.username}</span>
                    </div>
                    <span>•</span>
                    ...
                    {post.tags && post.tags.length > 0 && (
                        <>
                            ...
                            <div className="flex items-center gap-1.5">
                                {post.tags.map(tag => (
                                    <Badge key={tag} variant="outline" className="border-border text-zinc-400 font-normal">{tag}</Badge>
                                ))}
                            </div>
                        </>
                    )}
            ...
            <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0 bg-transparent rounded-none">
                    ...
                    <div className="mt-8 border-t border-border pt-6">
                        {!aiSummary ? (
                            <Button onClick={requestAISummary} isLoading={summarizing} variant="outline" className="border-primary/50 text-primary hover:bg-primary/10">
                                <Sparkles className="h-4 w-4 mr-2" /> Request AI Analysis
                            </Button>
                        ) : (
                            <div className="bg-primary/5 border border-primary/20 rounded-xl p-6">
                                <h4 className="flex items-center gap-2 text-primary font-bold mb-3"><Sparkles className="h-4 w-4" /> AI Mentor Analysis</h4>
                                <div
                                    className="prose prose-sm prose-invert max-w-none text-zinc-300"
                                    // primitive markdown
                                    dangerouslySetInnerHTML={{ __html: typeof aiSummary === 'string' ? aiSummary.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') : aiSummary.message }}
                                />
                            </div>
                        )}
                    </div>
            ...
            <div className="mt-12 pt-8 border-t border-border/80">
                <h3 className="text-xl font-bold text-white mb-6">{post.comments?.length || 0} Answers & Comments</h3>
