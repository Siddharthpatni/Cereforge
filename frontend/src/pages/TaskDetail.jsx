...
export function TaskDetail() {
    ...
    useEffect(() => {
        apiClient.get(`/tasks/${slug}`)
            .then(res => setTask(res.data))
            .catch(err => {
                addToast({ title: 'Error', message: 'Failed to load task', type: 'error' });
                navigate('/tasks');
            })
    ...
    if (loading) return <div className="p-12 text-center text-primary animate-pulse">Initializing Environment...</div>;
    if (!task) return null;
    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto">
            ...
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="sm" onClick={() => navigate('/tasks')} className="text-zinc-400 hover:text-white px-2">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div>
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="text-sm font-medium text-zinc-500 capitalize">{task.difficulty}</span>
                            {task.is_completed && (
                        ...
                        <CardContent className="p-6 md:p-8 prose prose-invert max-w-none">
                            <div
                                dangerouslySetInnerHTML={{
                                    __html: task.beginner_guide.replace(/\n\n/g, '<br/><br/>').replace(/### (.*?)\n/g, '<h3>$1</h3>').replace(/`([^`]+)`/g, '<code>$1</code>')
                                }}
                            />
                        </CardContent>
                    </Card>
                    ...
                    {/* Submission Form */}
                    <Card id="submit" className={task.is_completed ? "border-success/30 bg-success/5" : ""}>
                        <CardContent className="p-6">
                            <h3 className="text-lg font-semibold text-white mb-4">
                                {task.is_completed ? "Submission Accepted" : "Submit Your Solution"}
                            </h3>
                            ...
                            {task.is_completed ? (
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    <p className="text-sm text-zinc-400 mb-2">Paste your code, Colab link, or GitHub Gist URL below.</p>
                                    <textarea
                                        value={solutionRaw}
