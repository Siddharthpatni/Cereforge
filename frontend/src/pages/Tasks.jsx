...
export function Tasks() {
    ...
    const fetchTasks = async () => {
        setLoading(true);
        try {
            const url = selectedTrack === 'all' ? '/tasks' : `/tasks?track=${selectedTrack}`;
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Challenge Forge</h1>
                    <p className="text-zinc-400 mt-1">Select an AI challenge, read the guide, and submit your solution.</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTasks.map((task) => (
                        <Link key={task.id} to={`/tasks/${task.slug}`} className="block group">
                                ...
                                <CardHeader className="pb-3">
                                    <div className="flex items-start justify-between">
                                        ...
                                        {task.is_completed ? (
                                            ...
                                            <div className="text-xs font-mono text-primary font-bold">+{task.xp_reward} XP</div>
                                        )}
                                    </div>
                                    <CardTitle className="text-xl mt-3 text-white group-hover:text-primary transition-colors line-clamp-1">{task.title}</CardTitle>
                                </CardHeader>
                                <CardContent>
                                    ...
                                    <div className="mt-4 flex items-center justify-between text-xs text-zinc-500">
                                        <span>Difficulty: <span className="text-zinc-300 font-medium capitalize">{task.difficulty}</span></span>
                ...
                <div className="text-center p-12 border border-dashed border-zinc-800 rounded-xl bg-zinc-900/20">
                    <p className="text-zinc-400">No tasks found matching your criteria.</p>
                    <Button variant="ghost" className="mt-4" onClick={() => { setSearch(''); setSelectedTrack('all'); }}>
                        Clear Filters
                    </Button>
                </div>
            )}
