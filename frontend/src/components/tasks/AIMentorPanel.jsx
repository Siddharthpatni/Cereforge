...
export function AIMentorPanel({ taskId }) {
    const [messages, setMessages] = useState([
        { role: 'assistant', content: "Hello! I'm your AI Mentor for this challenge. How can I help you understand the requirements or get started?" }
    ]);
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            const res = await apiClient.post(`/ai/mentor/guidance?task_id=${taskId}`, {
                prompt: userMessage,
            ...
            setMessages(prev => {
                const newArr = [...prev];
                newArr[newArr.length - 1].content = typeof fullText === 'string' ? fullText : fullText.message || "I couldn't process that.";
                return newArr;
            });
            ...
            setMessages(prev => {
                const newArr = [...prev];
                newArr[newArr.length - 1].content = "I'm having trouble connecting to my neural net right now. Please try again.";
                return newArr;
            });
        } finally {
            setLoading(false);
    ...
    return (
        <div className="flex flex-col h-[500px] border border-border bg-card rounded-xl overflow-hidden glass-panel">
            <div className="flex items-center gap-2 bg-zinc-900/80 p-3 border-b border-border text-sm font-semibold">
                ...
                AI Mentor <span className="text-[10px] text-zinc-500 font-mono font-normal ml-auto">gpt-4o / claude</span>
            </div>
                ...
                {messages.map((msg, i) => (
                    <div key={i} className={cn("flex gap-3", msg.role === 'user' ? "flex-row-reverse" : "")}>
                        <div className={cn(
                            ...
                            msg.role === 'user' ? "bg-zinc-800 border-zinc-700" : "bg-primary/20 border-primary/30 text-primary"
                        )}>
                            {msg.role === 'user' ? <div className="text-white text-xs">U</div> : <Bot className="h-4 w-4" />}
                        </div>

                        <div className={cn(
                            "px-3 py-2 rounded-lg max-w-[80%] text-sm",
                            msg.role === 'user' ? "bg-zinc-800 text-white rounded-tr-none" : "bg-zinc-900/50 text-zinc-300 rounded-tl-none border border-zinc-800"
                        )}>
